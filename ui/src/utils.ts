...
export function extractJsonKeys(data: any[], maxDepth = 3): string[] {
...
export function getDefaultLayerName(jsonKey: string): string {
...
export function evaluateValueBuilder(builder: any, data: any): string {
  if (!builder || !builder.parts || builder.parts.length === 0) return '';
  
  return builder.parts.map((part: any) => {
    switch (part.type) {
      case 'text':
        return part.value || '';
      case 'key':
...
export function setupDragAndDrop(dropZone: HTMLElement, onFileDrop: (file: File) => void): void {
  if (!dropZone) return;
